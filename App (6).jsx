...
import {
  Trophy, Camera, PlusCircle, Compass, Home, User, Flame,
  CheckCircle2, Loader2, MapPin, Star, Zap, X, Mail, Lock,
  ShieldCheck, AlertCircle, LockKeyhole, Navigation, RefreshCw,
  Sparkles, Users, Map, Award, Swords, Heart, Target, Calendar,
  ChevronRight, Plus, Check, Crown, Bolt
} from 'lucide-react';
...
const MapTab = ({ quests, userLocation, gpsStatus, mockOffset, setMockOffset, QUEST_LAT, QUEST_LNG }) => {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      const L = window.L;
      const map = L.map(mapRef.current, { zoomControl: false }).setView([QUEST_LAT, QUEST_LNG], 15);
  ...
  useEffect(() => {
    ...
    if (userLocation) {
      const userIcon = L.divIcon({
        html: `<div style="width:16px;height:16px;background:#6366f1;border:3px solid white;border-radius:50%;box-shadow:0 0 12px #6366f1aa"></div>`,
        className: '', iconSize: [16, 16], iconAnchor: [8, 8]
      });
      const m = L.marker([userLocation.lat, userLocation.lng], { icon: userIcon }).addTo(leafletMap.current);
      markersRef.current.push(m);
    }

    // クエスト地点
    quests.filter(q => q.type === 'location').forEach(q => {
      ...
      const color = unlocked ? '#10b981' : '#f59e0b';
      const icon = L.divIcon({
        html: `<div style="background:${color};border:3px solid white;border-radius:50% 50% 50% 0;width:24px;height:24px;transform:rotate(-45deg);box-shadow:0 4px 12px ${color}88"><div style="transform:rotate(45deg);width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:10px">📍</div></div>`,
        className: '', iconSize: [24, 24], iconAnchor: [12, 24]
      });
      const popup = L.popup({ className: 'custom-popup' }).setContent(`<div style="font-family:sans-serif;padding:4px"><b>${q.title}</b><br/><span style="color:${color}">${unlocked ? '✅ 解放済' : `📏 ${formatDistance(dist)}`}</span><br/><span style="color:#f59e0b">⚡+${q.xp} XP</span></div>`);
      const m = L.marker([q.lat, q.lng], { icon }).bindPopup(popup).addTo(leafletMap.current);
      const circle = L.circle([q.lat, q.lng], { radius: q.radius, color, fillColor: color, fillOpacity: 0.1, weight: 2, dashArray: '6,4' }).addTo(leafletMap.current);
      markersRef.current.push(m, circle);
    });
  }, [userLocation, quests]);

  return (
    <div className="flex flex-col h-full">
      <h3 className="font-black text-lg text-slate-800 tracking-tight mb-3 text-left px-4 pt-2">クエストマップ</h3>
      {gpsStatus === 'mock' && (
        <MockGPSPanel mockOffset={mockOffset} setMockOffset={setMockOffset} QUEST_LAT={QUEST_LAT} QUEST_LNG={QUEST_LNG} />
      )}
      <div ref={mapRef} className="flex-1 rounded-2xl overflow-hidden mx-4 mb-4" style={{ minHeight: 320 }} />
      <div className="px-4 pb-2 space-y-2">
        {quests.filter(q => q.type === 'location').map(q => {
          const dist = userLocation ? calculateDistance(userLocation.lat, userLocation.lng, q.lat, q.lng) : null;
          const unlocked = dist !== null && dist <= q.radius;
          return (
            <div key={q.id} className={`flex items-center gap-3 p-3 rounded-xl border-2 ${unlocked ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-slate-100'}`}>
              <MapPin size={18} className={unlocked ? 'text-emerald-500' : 'text-slate-400'} />
              <div className="flex-1 text-left">
                <p className="font-bold text-sm text-slate-800">{q.title}</p>
                <p className={`text-xs font-bold ${unlocked ? 'text-emerald-600' : 'text-slate-400'}`}>{unlocked ? '✅ 範囲内' : dist ? `${formatDistance(dist)} 先` : '---'}</p>
              </div>
    ...
    <div className={`border-2 ${rank.border} rounded-2xl p-4 mb-3 opacity-70 text-left`}>
      <div className="flex gap-3 items-start">
        <div className={`w-10 h-10 rounded-xl ${rank.bg} flex items-center justify-center shrink-0`}>
          <LockKeyhole size={18} className={rank.color} />
        </div>
        <div className="flex-1">
